...
public class PlayerstationLineupCommand extends ParallelCommandGroup {
  ...
  public PlayerstationLineupCommand(DriveTrainSubsystem p_drivetrainSubsystem, LimelightSubsystem p_limelightSubsystem, ScoringSubsystem p_scoringSubsystem, IntakeManager p_intakeManager, ArmSubsystem p_armSubsystem, ViennaPIDController p_pidController) {
    ...
    if (m_drivetrainSubsystem.isAngleAcceptable()) {
      addCommands(
        ...
        new ArmToAngleCommand(m_armSubsystem, m_pidController, m_scoringSubsystem, ArmConstants.PLAYERSTATION)
      );
    }
  }
    ...
    ArrayList<PoseTriplet> moveForwardTrajectoryArray =
        new ArrayList<PoseTriplet>(
            Arrays.asList(
                ...
                new PoseTriplet(0, desiredDistance, 0)
            )
        );
